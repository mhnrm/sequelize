        ...
        options.include.forEach(function(include) {
          ...
          if (include.association.connectorDAO) {
            query += Utils._.template(joinQuery)({
              ...
              as:         this.quoteIdentifiers(include.association.connectorDAO.tableName),
              tableLeft:  this.quoteIdentifiers(include.association.connectorDAO.tableName),
              attrLeft:   this.quoteIdentifiers(include.association.identifier),
              tableRight: this.quoteIdentifiers(tableName),
              attrRight:  this.quoteIdentifiers('id')
            })
            
            query += Utils._.template(joinQuery)({
              table:      this.quoteIdentifiers(include.daoFactory.tableName),
              as:         this.quoteIdentifiers(include.as),
              tableLeft:  this.quoteIdentifiers(include.association.connectorDAO.tableName),
              attrLeft:   this.quoteIdentifiers(Object.keys(include.association.connectorDAO.primaryKeys)[0]),
              tableRight: this.quoteIdentifiers(include.as),
              attrRight:  this.quoteIdentifiers('id')
            })
          }
          else {
